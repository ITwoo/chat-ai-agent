    ...
    private createExpenseComparisonTool(context: AgentToolContext) {
        return tool(
            ...
            },
            {
                ...
                description: '두 개의 특정 기간을 직접 비교하여 지출 총액, 증감률, 카테고리별 증감을 확인할 때 사용한다. 이번 달과 지난달처럼 두 기간의 차이를 묻는 경우 사용한다.',
                schema: z.object({
    ...
    private createExpenseTrendTool(context: AgentToolContext) {
        return tool(
            ...
            },
            {
                ...
                description: '여러 시점에 걸친 지출 변화 흐름을 일별 또는 월별로 조회하고 이동평균을 확인할 때 사용한다. 최근 몇 달 동안 소비가 증가하거나 감소했는지처럼 시간에 따른 추세를 확인할 때 사용한다.',
                schema: z.object({
